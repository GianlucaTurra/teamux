...
func TestWindow_openAndCascade(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			target:  &testSession,
			w:       Window{Name: "TestW", WorkingDirectory: "~/"},
			wantErr: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := Pane{Name: "TestP", WorkingDirectory: "~/", SplitRatio: 25, splitDirection: 1}
			p2 := Pane{Name: "TestP2", WorkingDirectory: "~/", SplitRatio: 50, splitDirection: 0}
